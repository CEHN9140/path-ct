    ...
    for state, members in states.items():
        ...
        for pathway in pathways:
            ...
            if not patients or not rest:
                continue
            selected = patients + rest; y = np.asarray([float(scores.loc[patient, pathway]) for patient in selected]); is_state = np.asarray([int(patient in patients) for patient in selected]); tss = [tss_code(patient) for patient in selected]; levels = sorted(set(tss)); unadjusted_design = sm.add_constant(is_state, has_constant="add"); unadjusted_fit = sm.OLS(y, unadjusted_design).fit(); design = np.column_stack([np.ones(len(selected)), is_state, *[np.asarray([int(value == level) for value in tss]) for level in levels[1:]]]); fit = sm.OLS(y, design).fit(); unadjusted_beta = float(unadjusted_fit.params[1]); beta = float(fit.params[1]); ci = fit.conf_int()[1]; current.append({"state_id": state, "pathway": pathway, "state_n": len(patients), "rest_n": len(rest), "tss_level_n": len(levels), "unadjusted_beta": base.rounded(unadjusted_beta), "unadjusted_smd": base.rounded(base.numeric_test([float(scores.loc[patient, pathway]) for patient in patients], [float(scores.loc[patient, pathway]) for patient in rest])[0]), "tss_adjusted_beta": base.rounded(beta), "tss_adjusted_ci_low": base.rounded(ci[0]), "tss_adjusted_ci_high": base.rounded(ci[1]), "tss_adjusted_p_value": base.rounded(fit.pvalues[1]), "effect_attenuation": base.rounded(1 - abs(beta) / abs(unadjusted_beta)) if unadjusted_beta else None, "tss_levels": ";".join(levels)})
        for row, q_value in zip(current, base.bh([row["tss_adjusted_p_value"] for row in current])): row["tss_adjusted_q_value"] = q_value
        rows.extend(current)
    return rows


def leave_out_tss_rna(states, scores, pathways, macro_states, stable_ids, excluded_tss, config_dir):
    available = [patient for patient in stable_ids if patient in scores.index and tss_code(patient) != excluded_tss]
...
def run(data_root=Path("output_kirc"), stable_root=Path("output_kirc_v12/03_multi_k_accepted_core_stability_v11"), output_root=Path("output_kirc_v12/06_macro_state_analysis"), config_dir=Path("configs"), top_pathways=25, top_cnv=25, random_state=42, force=False):
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--data-root", type=Path, default=Path("output_kirc")); parser.add_argument("--stable-root", type=Path, default=Path("output_kirc_v12/03_multi_k_accepted_core_stability_v11")); parser.add_argument("--output-root", type=Path, default=Path("output_kirc_v12/06_macro_state_analysis")); parser.add_argument("--config-dir", type=Path, default=Path("configs")); parser.add_argument("--top-pathways", type=int, default=25); parser.add_argument("--top-cnv", type=int, default=25); parser.add_argument("--random-state", type=int, default=42); parser.add_argument("--force", action="store_true"); print(json.dumps(run(**vars(parser.parse_args())), ensure_ascii=False, indent=2))
