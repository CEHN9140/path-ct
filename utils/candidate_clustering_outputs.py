...
def select_pac_stability_k(
    consensus_records: list[Mapping[str, Any]],
    *,
    min_cluster_size: int = 10,
    pac_tie_tolerance: float = 0.02,
) -> dict[str, Any]:
    diagnostics = []
    eligible_records = []
    for record in consensus_records:
        ...
        sizes = [
            int(value)
            for value in list(
                item.get("cluster_sizes", [])
                or cluster_sizes_from_labels(item.get("labels", []))
            )
        ]
        min_size = min(sizes) if sizes else 0
        valid = bool(sizes) and min_size >= int(min_cluster_size)
        diagnostic = {
            "n_clusters": int(item.get("n_clusters", 0) or 0),
            "pac": float(item.get("pac", 0.0) or 0.0),
            "delta_area": float(item.get("delta_area", 0.0) or 0.0),
            "consensus_silhouette": item.get("consensus_silhouette"),
            "cluster_sizes": sizes,
            "eligible": valid,
            "excluded_reason": ""
            if valid
            else f"minimum cluster size {min_size} is below {int(min_cluster_size)}",
        }
        diagnostics.append(diagnostic)
        if valid:
            eligible_records.append({**item, "cluster_sizes": sizes})
    if not eligible_records:
        best = dict(consensus_records[0]) if consensus_records else {}
        return {
            **best,
            "selection_metric": "pac_stability",
            "candidate_k_diagnostics": diagnostics,
            "selected_k_reason": (
                f"No eligible K passed minimum cluster size {int(min_cluster_size)}; "
                "falling back to the first available consensus record."
            ),
        }
    min_pac = min(float(item.get("pac", 0.0) or 0.0) for item in eligible_records)
    tied = [
        item
        for item in eligible_records
        if float(item.get("pac", 0.0) or 0.0) <= min_pac + float(pac_tie_tolerance)
    ]

    def sort_key(item: Mapping[str, Any]) -> tuple[float, int]:
        silhouette = item.get("consensus_silhouette")
        try:
            silhouette_value = float(silhouette)
        except (TypeError, ValueError):
            silhouette_value = -1.0
        return (-silhouette_value, int(item.get("n_clusters", 0) or 0))

    selected = dict(sorted(tied, key=sort_key)[0])
    selected["selection_metric"] = "pac_stability"
    selected["candidate_k_diagnostics"] = diagnostics
    selected["selected_k_reason"] = (
        f"Selected K={int(selected.get('n_clusters', 0) or 0)} by lowest eligible PAC "
        f"within minimum cluster size {int(min_cluster_size)}; PAC ties within "
        f"{float(pac_tie_tolerance):.3f} were resolved by higher consensus silhouette "
        "and then smaller K."
    )
    return selected
