...
def q_below(row, threshold=.05):
    q_value = finite(row.get("q_value"))
    return q_value is not None and q_value < threshold
...
def load_main_sets(data_root: Path) -> list[dict]:
    return load_review_sets(data_root / "subtype_review/final_review_summary.json")


def load_review_sets(summary_path: Path) -> list[dict]:
...
def select_pathways(rows, limit):
    grouped = {pathway: [row for row in rows if row["pathway"] == pathway] for pathway in sorted({row["pathway"] for row in rows})}; significant = [pathway for pathway, values_for_pathway in grouped.items() if any(q_below(row) for row in values_for_pathway)]; order = lambda pathway: (-max(abs(row.get("smd") or 0) for row in grouped[pathway]), pathway); selected = sorted(significant, key=order)[:limit]; selected.extend(pathway for pathway in sorted((x for x in grouped if x not in selected), key=order)[:limit - len(selected)]); return selected


def select_cnv_heatmap_features(rows, limit):
    grouped = {}
    for row in rows: grouped.setdefault(row["feature"], []).append(row)
    return sorted(grouped, key=lambda feature: (not any(q_below(row) for row in grouped[feature]), -max(abs(row.get("cliffs_delta") or 0) for row in grouped[feature]), feature))[:limit]
...
def update_pairwise_similarity(rows, rna_pair, wxs_pair, cnv_pair):
    for row in rows:
        pair = lambda source: [item for item in source if item["core_a"] == row["core_a"] and item["core_b"] == row["core_b"]]; rna, wxs, cnv = pair(rna_pair), pair(wxs_pair), pair(cnv_pair); row["rna_pairwise_fdr_pathway_count"] = sum(q_below(item) for item in rna); row["wxs_pairwise_fdr_gene_count"] = sum(q_below(item) for item in wxs); row["cnv_pairwise_fdr_feature_count"] = sum(q_below(item) for item in cnv)
    return rows
...
def plot_fused_structure_comparison(path, similarity, ids, cores, random_state):
    from utils.visualization import configure_matplotlib
    configure_matplotlib(); import matplotlib.pyplot as plt
    from sklearn.manifold import SpectralEmbedding
    core_ids = [case_id for core in sorted(cores) for case_id in sorted(cores[core]) if case_id in ids]; indices = [ids.index(case_id) for case_id in core_ids]; normalized = affinity_characterization(similarity, ids, cores)["similarity"][np.ix_(indices, indices)]; distance = np.maximum(0, 1 - normalized)
    centered = -.5 * distance ** 2; centered -= centered.mean(axis=0, keepdims=True); centered -= centered.mean(axis=1, keepdims=True); centered += centered.mean(); eigenvalues, eigenvectors = np.linalg.eigh(centered); positive = np.flatnonzero(eigenvalues > 1e-10)[::-1]; pcoa = eigenvectors[:, positive[:2]] * np.sqrt(eigenvalues[positive[:2]]); explained = eigenvalues[positive[:2]] / eigenvalues[positive].sum()
    spectral = SpectralEmbedding(n_components=2, affinity="precomputed", random_state=random_state).fit_transform(normalized)
    degree = normalized.sum(axis=1); diffusion_operator = normalized / np.sqrt(np.outer(degree, degree)); diffusion_values, diffusion_vectors = np.linalg.eigh(diffusion_operator); order = np.argsort(diffusion_values)[::-1][1:3]; diffusion = diffusion_vectors[:, order] * diffusion_values[order]
    labels = {case_id: core for core, members in cores.items() for case_id in members}; colors = {core: plt.get_cmap("tab10")(index) for index, core in enumerate(sorted(cores))}; projections = [(pcoa, f"PCoA (first two positive axes: {explained.sum():.1%})", "PCoA-1", "PCoA-2"), (spectral, "Spectral embedding", "Spectral-1", "Spectral-2"), (diffusion, "Diffusion map", "DC-1", "DC-2")]
    figure, axes = plt.subplots(1, 3, figsize=(15, 4.8))
    for axis, (coordinates, title, xlabel, ylabel) in zip(axes, projections):
        for core in sorted(cores):
            selected = [index for index, case_id in enumerate(core_ids) if labels.get(case_id) == core]
            axis.scatter(coordinates[selected, 0], coordinates[selected, 1], label=core, color=colors[core], s=24)
        axis.set_title(title); axis.set_xlabel(xlabel); axis.set_ylabel(ylabel); axis.grid(alpha=.15)
    handles = [plt.Line2D([], [], marker="o", linestyle="", color=colors[core], label=core) for core in sorted(cores)]; figure.suptitle(f"Fused affinity structure of stable cores (n={len(core_ids)}; exploratory)"); figure.legend(handles=handles, title="CORE", bbox_to_anchor=(1.01, .8), loc="upper left"); figure.tight_layout(); save_figure(figure, path); return ["pcoa", "spectral", "diffusion"]
...
def run(data_root: Path, experiment_root: Path, output_root: Path, config_dir: Path, top_pathways=25, top_cnv=25, random_state=42, force=False, affinities_override=None, affinity_ids_override=None, primary_root=None):
    ...
    if primary_root is not None:
        primary_sets = load_review_sets(Path(primary_root) / "final_review_summary.json")
        primary_mapping, primary_composition = main_mapping(cores, primary_sets)
        write_csv(output_root / "five_view_core_to_five_view_primary_mapping.csv", primary_mapping)
        write_csv(output_root / "five_view_core_to_five_view_primary_composition.csv", [{"core_id": core, **counts} for core, counts in primary_composition.items()])
        mapping_rows, composition, main_sets = primary_mapping, primary_composition, primary_sets
