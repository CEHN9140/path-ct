...
def build_recurrent_sets(observations: list[dict[str, Any]], min_subtype_size: int) -> list[dict[str, Any]]:
    records = []
...
def write_recurrent_outputs(output_dir: Path, observations: list[dict[str, Any]], recurrent_sets: list[dict[str, Any]], *, aggregation_manifest: Mapping[str, Any], aggregation_signature: str, run_count: int, min_subtype_size: int) -> dict[str, Any]:
    pd.DataFrame([
    ...
    summary = {"status": "complete", "analysis_type": "closed_recurrent_accept_set_statistics", "aggregation_signature": aggregation_signature, "run_count": run_count, "total_accept_set_count": len(observations), "min_subtype_size": min_subtype_size, "recurrent_set_count": len(recurrent_sets), "automatic_subtype_selection": False}
    write_json(output_dir / "aggregation_manifest.json", {**aggregation_manifest, "aggregation_signature": aggregation_signature})
    write_json(output_dir / "aggregation_summary.json", summary)
    return summary


def run_multi_k_aggregation(output_root: str, config: Mapping[str, Any], input_signature: str) -> dict[str, Any]:
    ...
    for initial_k in initial_ks:
        for repeat in repeats:
            ...
            metadata_path, summary_path, sets_path = (run_root / name for name in ("run_metadata.json", "final_review_summary.json", "final_subtype_sets.json"))
            ...
            if metadata.get("input_signature") != input_signature or metadata.get("status") != "complete" or summary.get("raw_control_status") != "complete" or summary.get("status") != "review_complete":
                raise ValueError(f"Review run is incomplete or has mismatched inputs: {run_root}")
            run_files.append((initial_k, repeat, sets_path))
    ...
    recurrent_sets = build_recurrent_sets(observations, int(params["min_subtype_size"]))
    return write_recurrent_outputs(output_dir, observations, recurrent_sets, aggregation_manifest=aggregation_manifest, aggregation_signature=aggregation_signature, run_count=len(run_files), min_subtype_size=int(params["min_subtype_size"]))
...
def main() -> None:
    ...
    params = config["multi_k"]
    first_run = Path(args.output_root) / "subtype_review" / "runs" / f"K{params['initial_ks'][0]}" / f"repeat{params['repeats'][0]}" / "run_metadata.json"
    input_signature = json.loads(first_run.read_text(encoding="utf-8"))["input_signature"]
    print(json.dumps(run_multi_k_aggregation(args.output_root, config, input_signature), ensure_ascii=False, indent=2))
