...
Accept requires a supporting `biological_support` finding for the target set and no unresolved supported structural correction. This is the admission requirement for a biologically interpretable new subtype in this project; an inconclusive or unavailable biological finding cannot be converted into acceptance by the absence of contradiction. Structural, cross-modal, confounder and known-label evidence may strengthen or block the decision, but cannot replace positive biological support. If biological support has been attempted and remains inconclusive, and no further decision-relevant evidence can be acquired, choose Drop rather than Accept.
...
Python validates JSON types, identifiers, metric references, partition signatures, tool reuse, budgets, plan identifiers, member completeness, mutual exclusivity and Merge pairwise evidence. A scientific action is rejected when its target has an unattempted decision-relevant gap, when an unattempted whole-partition gap exists, when Accept lacks a target-specific supporting `biological_support` finding, or when Split/Merge lacks a matching conflicting structural finding. A failed capability must be reported as unavailable or inconclusive and prevents final completion; failure is never positive evidence for Accept. An attempted but inconclusive dimension cannot be acquired again for the same partition. Python does not calculate scientific metrics or choose among valid scientific actions; it enforces these explicit protocol contracts.
