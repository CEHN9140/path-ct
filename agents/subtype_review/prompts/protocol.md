...
After base evidence is complete, Python accepts sets that already have identity support in at least two original modalities and no deterministic technical invalidation. Mixed confounder association is retained as a caveat, not an Accept veto. Structural review is generated only for sets that still fail Accept: Split additionally requires enough patients for two minimum-size children, and Merge requires at least one non-Accept endpoint. Two independently acceptable sets are never merged speculatively.

An exact Split candidate requires support for its child membership in at least two original modalities, molecular/biology support, and no technical or known-label veto. Imaging-only Split requests biology evidence. Merge requires positive weak-boundary evidence in at least two modalities and no biological distinction veto. Python separates partition-level confounder association, singleton set-level association, and technical invalidation. Drop is permitted for exact technical invalidation or for insufficient identity support after every authorized Split/Merge intent involving that set has been exhausted. The recorded reason distinguishes these cases. Drop never removes patients.
