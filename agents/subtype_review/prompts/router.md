...
For evidence acquisition, use an empty target list for a complete-partition request or targets contained in the matching gap. For scientific actions, use exactly one target and set `dimension` to null. Reviser selects the exact Split or Merge plan.
