...
Accept requires supporting `cross_modal_consistency` at `set_identity` in at least two original modalities. Biology can be supporting, mixed, or inconclusive. Set-level confounder association can block Accept but cannot justify Drop; Drop requires Python-derived technical invalidation for that exact singleton set. Partition-level conflict does not justify dropping an individual set. A tool failure, weak single modality, biology inconclusive result, budget pressure, or non-significant p/q cannot justify Drop.
