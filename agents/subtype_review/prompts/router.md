...
\`need_more_evidence\` is allowed only for a specific decision-critical evidence gap that can be reduced by an available extra tool. Each exact tool and target must appear in \`available_extra_evidence\`; do not infer availability from the Evidence Reports. Weak, mixed, or inconclusive evidence alone is not a reason to request more evidence. If a target has no available extra evidence, choose \`accept\`, \`drop\`, \`split\`, or \`merge\`; if the evidence is insufficient for Accept, Split, or Merge, choose Drop. Python executes only valid tool requests and then performs a new full round.
