...
When requesting evidence, each entry must match the `EvidenceRequest` schema exactly and contain only `dimension`, `scope`, `target_ids`, and `question`. Write one request per dimension/scope/target combination; do not add `aspects` or `reason`. The EvidenceRequest question must describe an unresolved scientific question, not prescribe an analysis plan. Do not name tools or enumerate evidence types merely because they exist. For example, ask whether a candidate has a coherent and interpretable biological phenotype relative to the current partition, rather than asking to assess pathway and mutation enrichment. Ask whether measured technical or acquisition factors could plausibly account for an important part of the partition signal, rather than asking to run confounder analyses. You may ask a scientifically specific follow-up based on existing Evidence Reports, such as whether a phenotype is also associated with a distinct somatic alteration pattern. The Verifier decides which eligible tool should answer the question.
