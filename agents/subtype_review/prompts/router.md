...
Do not request set-scope confounder evidence merely because a partition-level association exists. Request candidate-specific follow-up only when resolving whether the broader concern materially applies to that candidate could change its disposition. If independent membership is already not positively justified by other decision-relevant evidence, terminal drop does not require proving a candidate-specific confounder.
