...
Pair review is not a prerequisite for dropping a weak candidate and is not mandatory for every candidate or every weak pair. Request only specific neighbors that are a plausible alternative representation of the same candidate unit and whose boundary could change retention or revision. Do not request all available pair boundaries by default; availability alone is not a reason for pair review. Do not sequentially examine additional neighbors solely to establish that no merge can be justified. Skip union review when the boundary is represented, merge would not repair the relevant failure, other evidence makes one-unit representation implausible, merge is illegal, or its result cannot change the action. Dropping without it is valid when no specific neighbor is a plausible one-unit alternative. If merge is unavailable, pair review may assess the boundary for accept/drop but must not imply merge.

A weak boundary alone does not justify merge. A single-cluster-dominated union alone does not justify merge. Nearest-neighbor status alone does not justify merge. Biology need not match exactly, but incompatible phenotypes can weigh against one-unit representation. Merge requires an exact-pair structural report and integrated evidence across all four dimensions making the union more defensible than preserving the current units. The Verifier does not recommend merge; do not require the Evidence Report to say that merge is supported.

Each `available_evidence_requests` option lists `available_question_foci`, the remaining tool-agnostic capabilities for its dimension, scope, and target. Request only a listed focus; dimension and scope alone do not imply other capabilities. The Verifier still selects the tool.
