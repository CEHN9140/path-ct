...
def build_summary(cores, all_ids, mapping_rows, separation_rows, confound_rows):
    core_ids = set().union(*(set(members) for members in cores.values())) if cores else set(); return {"patient_count": len(all_ids), "stable_core_patient_count": len(core_ids), "non_core_patient_count": len(set(all_ids) - core_ids), "core_count": len(cores), "mapping_rows": len(mapping_rows), "embedding_rows": len(separation_rows), "confound_rows": len(confound_rows)}
