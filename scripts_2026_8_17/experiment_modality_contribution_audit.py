...
def run(data_root=Path("output_kirc"), stable_root=Path("output_kirc_v11/experiment_multi_k_accepted_core_stability"), output_root=Path("output_kirc_v11/experiment_modality_contribution_audit"), config_dir=Path("configs"), k_values=range(2, 9), knn=10, force=False):
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--data-root", type=Path, default=Path("output_kirc")); parser.add_argument("--stable-root", type=Path, default=Path("output_kirc_v11/experiment_multi_k_accepted_core_stability")); parser.add_argument("--output-root", type=Path, default=Path("output_kirc_v11/experiment_modality_contribution_audit")); parser.add_argument("--config-dir", type=Path, default=Path("configs")); parser.add_argument("--knn", type=int, default=10); parser.add_argument("--force", action="store_true"); args = parser.parse_args(); print(json.dumps(run(**vars(args)), ensure_ascii=False, indent=2))
