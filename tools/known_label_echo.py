...
def cluster_labels_by_case(all_cluster_states, fallback_cluster_state):
    ...
    for state in states:
        cluster_id = str(state.get("set_id") or state.get("cluster_id") or "")
...
def known_label_echo_test(
    ...
    limitations = [
        "Only TCGA stage and grade labels are assessable in the current data.",
        "Published molecular ccRCC subtype taxonomies were not evaluated.",
        "Low stage or grade overlap does not by itself establish molecular novelty.",
    ]
    decision_metrics = {
        "assessable_labels": ["stage", "grade"],
        "by_label": {
            label: {
            ...
            for label, comparison in structure_comparison.items()
        },
    ...
    metrics = {
        ...
        "decision_semantics": (
            "ARI, homogeneity, and completeness describe continuous overlap; "
            "stage and grade are assessed independently. AMI and optimal mapping "
            "accuracy remain full-audit metrics only."
        ),
    }
    return tool_result(
