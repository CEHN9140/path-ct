...
def top_folder_under_root(root: Path, file_path: Path) -> str:
    rel = file_path.absolute().relative_to(root.absolute())
    parts = rel.parts[:-1]
    ...
    for dp, _, files in os.walk(root):
        for name in files:
            ...
            if getattr(read_headers, "_verbose", False):
                ...
                progress_every = getattr(read_headers, "_progress_every", 0)
                if progress_every and count % progress_every == 0:
                    status_print(f"[headers] scanned {count} DICOM files; latest={p}")
...
def run_radqy_dataset(
    ...
    *,
    ...
    tag_files = {"mri": "mri-tags.yaml", "ct": "ct-tags.yaml"}
    yaml_path = config_dir / tag_files.get(str(scantype).strip().lower(), "mri-tags.yaml")
    if not yaml_path.exists():
        raise FileNotFoundError(f"{yaml_path.name} not found under {config_dir}")

    segmenter_fn, segmenter_name = resolve_segmenter(str(segmenter))
    tag_dict, abbr_order = read_yaml(yaml_path)
    read_headers._verbose = bool(verbose)  # type: ignore[attr-defined]
    read_headers._progress_every = 0  # type: ignore[attr-defined]
    df, _ = read_headers(root, tag_dict)
    if df.empty:
        table = pd.DataFrame(columns=["P#", "Participant (topfolder--subfolder--patient ID)", "NSL"])
    else:
        df_kept = keep_middle_percent(df, int(middle_percent))
        if int(num_samples) > 1:
            group_cols = ["TopFolder", "SubFolder", "PatientID"]
            df_kept = (
                df_kept.sort_values(by=["StudyInstanceUID", "SeriesInstanceUID", "InstanceNumber", "Path"], kind="mergesort")
                .groupby(group_cols, group_keys=False)
                .apply(lambda g: g.iloc[:: int(num_samples)].copy())
                .reset_index(drop=True)
            )
        ...
        rows = []
        groups = list(df_kept.groupby(["TopFolder", "SubFolder", "PatientID"], dropna=False))
        show_progress = sys.stderr.isatty()
        for idx, ((top, sub, pid), group) in enumerate(tqdm(groups, desc="RadQy participants", unit="series", disable=not show_progress), start=1):
            iqm_sum = defaultdict(float)
            iqm_n = defaultdict(int)
            iqm_invalid = defaultdict(int)
            tag_values = {abbr: first_nonempty_for_col(group, abbr) for abbr in abbr_order}
            sorted_group = group.sort_values(by=["Path"], kind="mergesort").reset_index(drop=True)
            if save_images:
                participant_dir = out_dir / safe_name(f"{top}--{sub}--{pid}")
                participant_dir.mkdir(parents=True, exist_ok=True)
            for row in sorted_group.itertuples(index=False):
                try:
                    ds = pydicom.dcmread(Path(row.Path), stop_before_pixels=False, force=True)
                    slice_values = _read_float32_slice(ds)
                    fg, bg = segmenter_fn(slice_values)
                    if save_images:
                        img_u8 = dcm_to_uint8(ds)
                        if img_u8 is not None:
                            PILImage.fromarray(img_u8).save(participant_dir / f"{safe_name(str(pid))}_{len(iqm_n) + 1}.png")
                    if save_fgbg and save_images:
                        PILImage.fromarray((fg.astype(np.uint8) * 255)).save(participant_dir / f"{safe_name(str(pid))}_fg.png")
                        PILImage.fromarray((bg.astype(np.uint8) * 255)).save(participant_dir / f"{safe_name(str(pid))}_bg.png")
                    fg_vals = slice_values[fg.astype(bool)]
                    bg_vals = slice_values[bg.astype(bool)]
                    for fn in iqm_funcs:
                        try:
            ...
            result_row = {
                "#": idx,
                "Participant (topfolder--subfolder--patient ID)": f"{top}--{sub}--{pid}",
                "NSL": len(group),
            }
            result_row.update(tag_values)
            for name in iqm_names:
                n = iqm_n.get(name, 0)
                result_row[name] = (iqm_sum[name] / n) if n > 0 else float("nan")
            result_row["FAIL_FRAC"] = int(sum(iqm_invalid.values()))
            rows.append(result_row)

        cols = ["#", "Participant (topfolder--subfolder--patient ID)", "NSL"] + list(abbr_order) + list(iqm_names) + ["FAIL_FRAC"]
        table = pd.DataFrame(rows)
        for column in cols:
            if column not in table.columns:
                table[column] = np.nan if column in iqm_names else ""
        table = table[cols]
        table.rename(columns={"#": "P#"}, inplace=True)
        table["P#"] = table["P#"].apply(lambda value: f"P{value}")

    header_lines = [
        f"#outdir: {out_dir}",
        f"#scantype: {str(scantype).strip().upper()}",
        f"#settings: inputdir={root} | middle_percent={middle_percent} | num_samples={num_samples} | segmenter={segmenter_name} | save_images={bool(save_images)}",
        "",
    ]
    ...
    return {
        "results_tsv_path": str(out_path),
        "output_dir": str(out_dir),
        "processed_count": int(len(table)),
        "iqm_table": table,
        "errors": [],
    }
...
def main():
    ...
    ap.add_argument("--outputdir", type=str, default=None, help="Output directory for radqy_results.tsv, thumbnails, and optional masks.")
    ap.add_argument("--header-progress-every", type=int, default=1000, help="Print header scan progress every N DICOM files; use 0 to disable.")
    ...
    folder_name = Path(args.outputdir).expanduser().resolve().name if args.outputdir else root.name

    # Default matches RadQy's frontend/Data layout; --outputdir supports fixed dataset workspaces.
    out_dir = Path(args.outputdir).expanduser().resolve() if args.outputdir else REPO_ROOT / "frontend" / "Data" / folder_name
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "radqy_results.tsv"

    # Load YAML and data
    tag_dict, abbr_order = read_yaml(yaml_path)
    # configure header scan verbosity
    read_headers._verbose = args.verbose  # type: ignore[attr-defined]
    read_headers._progress_every = args.header_progress_every if not args.verbose else 0  # type: ignore[attr-defined]
    df, _ = read_headers(root, tag_dict)
    status_print(f"[headers] completed: {len(df)} DICOM files loaded.")
    ...
    total_iqms_per_participant = len(iqm_funcs) + 1  # IQMs plus participant-level FAIL_FRAC
