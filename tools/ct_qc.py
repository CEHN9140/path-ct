...
def dicom_prefilter_reasons(series_summary: Mapping[str, Any], config: Mapping[str, Any]) -> list[str]:
    ...
    excluded_image_types = {str(item).upper() for item in config["excluded_image_types"]}
    required_image_types = {str(item).upper() for item in config["required_image_types"]}
    reasons = []
    ...
    if not isinstance(thickness, (int, float)) or thickness <= 0 or thickness > float(config["max_slice_thickness"]):
        reasons.append("The slice thickness is missing or out of range.")

    pixel_spacing = [series_summary.get("pixel_spacing_row"), series_summary.get("pixel_spacing_col")]
    if any(not isinstance(value, (int, float)) or value <= 0 or value > float(config["max_pixel_spacing"]) for value in pixel_spacing):
        reasons.append("The pixel spacing is missing or out of range.")

    normal_z = series_summary.get("normal_z_median")
    if not isinstance(normal_z, (int, float)) or abs(normal_z) < float(config["min_abs_normal_z"]):
        reasons.append("The series orientation is not axial enough.")
    if int(series_summary.get("rows") or 0) < int(config["min_matrix_size"]) or int(series_summary.get("columns") or 0) < int(config["min_matrix_size"]):
    ...
    if isinstance(z_gap_max, (int, float)) and isinstance(z_spacing_median, (int, float)) and z_spacing_median > 0:
        if z_gap_max > float(config["max_z_gap_factor"]) * z_spacing_median:
            reasons.append("The series has a large slice gap.")

    if int(series_summary.get("duplicate_z_count") or 0) > int(config["max_duplicate_slices"]):
...
def convert_dicom_series(series_path: Path, output_dir: Path, config: Mapping[str, Any]) -> tuple[str, list[str], list[str], list[str]]:
    ...
    generated_nifti = sorted(path for path in output_dir.glob("*.nii.gz") if path.resolve() not in existing_nifti)
    generated_json = sorted(path for path in output_dir.glob("*.json") if path.resolve() not in existing_json)
    usable_nifti = [path for path in generated_nifti if not any(token in path.name.lower() for token in ["_eq_", "mask", "seg", "label", "derived"])]
    if not usable_nifti:
...
def check_nifti_volume(nifti_path: str | Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        result["nifti_qc_reasons"].append("The NIfTI image cannot be read by SimpleITK.")
        return result
    ...
    result.update({"hu_min": float(np.min(values)), "hu_max": float(np.max(values)), "hu_p1": hu_p1, "hu_p99": hu_p99, "hu_range_p99_p1": hu_p99 - hu_p1})
    if result["hu_min"] < float(config["hu_min_allowed"]) or result["hu_max"] > float(config["hu_max_allowed"]):
        result["nifti_qc_reasons"].append("The HU value range is outside the allowed range.")
...
def check_totalsegmentator_rois(nifti_path: str | Path, output_dir: Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    command = [str(config["bin"]), "-i", str(nifti_path), "-o", str(output_dir), "-ta", str(config["task"]), "-q"]
    if bool(config["use_roi_subset"]):
        command.extend(["-rs", *rois])
    if config["device"]:
        device = str(config["device"])
        command.extend(["-d", device.replace("cuda:", "gpu:")])

    env = dict(os.environ)
    env["TOTALSEG_HOME_DIR"] = "/tmp/path_ct_totalseg_home"
    env["MPLCONFIGDIR"] = "/tmp/path_ct_matplotlib"
    completed = subprocess.run(command, env=env, capture_output=True, text=True, check=False)
    if completed.returncode != 0:
        message = (completed.stderr.strip() or completed.stdout.strip() or "TotalSegmentator failed to run.").splitlines()[-1]
        result["totalseg_reasons"].append(f"TotalSegmentator failed to run: {message}")
        return result
    ...
    if roi_masks:
        ...
        for color, roi in zip(colors, rois):
            ...
            rgb[mask_slice] = (0.55 * rgb[mask_slice] + 0.45 * np.asarray(color)).astype(np.uint8)
    ...
    min_single_roi_volume_ml = float(config.get("min_single_roi_volume_ml", config.get("min_total_roi_volume_ml", 0)))
    if any(float(value or 0) > min_single_roi_volume_ml for value in result["totalseg_roi_volumes_ml"].values()):
...
def prepare_ct_series(
    ...
    if report_path.is_file():
        try:
            ...
            existing_rows = {
                (int(row["record_index"]), str(row["source_path"])): {str(key): value for key, value in row.to_dict().items()}
                for _, row in existing_table.iterrows()
                if str(row.get("record_index", "")).strip()
            }
        except Exception:
            existing_rows = {}
    report_rows = []
    ...
    for index, ct_record in tqdm(list(enumerate(ct_records, start=1)), desc=f"{case_id} CT series", unit="series", leave=False, disable=not show_progress):
        source_path = Path(str(ct_record.get("File Path", "") or ""))
        cached_row = existing_rows.get((index, str(source_path)))
        if cached_row:
            cached_full_pass = all(str(cached_row.get(key, "")).strip().lower() == "true" for key in ["prefilter_pass", "nifti_qc_pass", "totalseg_pass"])
            converted_path = str(cached_row.get("converted_path", "") or "")
            totalseg_output_dir = str(cached_row.get("totalseg_output_dir", "") or "")
            if not cached_full_pass or (converted_path and Path(converted_path).is_file() and totalseg_output_dir and Path(totalseg_output_dir).is_dir()):
                report_rows.append(cached_row)
                pd.DataFrame(report_rows).to_csv(report_path, index=False)
                if cached_full_pass:
                    converted_file = Path(converted_path)
                    converted_name = converted_file.name
                    ct_id = converted_name[:-7] if converted_name.endswith(".nii.gz") else converted_file.stem
                    subject_id = safe_identifier(f"{case_id}__{ct_id.replace('.', '_')}")
                    try:
                        roi_voxels = json.loads(str(cached_row.get("totalseg_roi_voxels", "") or "{}"))
                    except json.JSONDecodeError:
                        roi_voxels = {}
                    try:
                        roi_volumes = json.loads(str(cached_row.get("totalseg_roi_volumes_ml", "") or "{}"))
                    except json.JSONDecodeError:
                        roi_volumes = {}
                    n_slices = pd.to_numeric(cached_row.get("n_slices"), errors="coerce")
                    slice_thickness = pd.to_numeric(cached_row.get("slice_thickness_median"), errors="coerce")
                    numeric_fields = {
                    ...
                    series_records.append(
                        {
                            ...
                            "file_size_mb": float(converted_file.stat().st_size / 1024**2) if converted_file.exists() else 0.0,
                            ...
                            "slice_thickness_median": float(slice_thickness) if pd.notna(slice_thickness) else None,
                            **{key: float(value) for key, value in numeric_fields.items() if pd.notna(value)},
                            "prefilter_pass": True,
                            "nifti_qc_pass": True,
                            "totalseg_pass": True,
                            "image_type_union": [item for item in str(cached_row.get("image_type_union", "") or "").split(";") if item],
                            "prefilter_reasons": [item for item in str(cached_row.get("prefilter_reasons", "") or "").split(";") if item],
                            "nifti_qc_reasons": [item for item in str(cached_row.get("nifti_qc_reasons", "") or "").split(";") if item],
                            "totalseg_reasons": [item for item in str(cached_row.get("totalseg_reasons", "") or "").split(";") if item],
                            "totalseg_output_dir": totalseg_output_dir,
                            "totalseg_overlay_png": str(cached_row.get("totalseg_overlay_png", "") or ""),
                            "totalseg_detected_rois": [item for item in str(cached_row.get("totalseg_detected_rois", "") or "").split(";") if item],
                            "totalseg_missing_rois": [item for item in str(cached_row.get("totalseg_missing_rois", "") or "").split(";") if item],
                            "totalseg_roi_voxels": roi_voxels,
                            "totalseg_roi_volumes_ml": roi_volumes,
                            "generated_nifti_files": [item for item in str(cached_row.get("generated_nifti_files", "") or "").split(";") if item],
                            "generated_json_files": [item for item in str(cached_row.get("generated_json_files", "") or "").split(";") if item],
                        }
                    )
                continue

        headers = read_dicom_headers(source_path) if source_path.is_dir() else []
        series_summary = summarize_dicom_series(headers, ct_record, prefilter_config) if headers else {
            "modality": "",
            "n_slices": 0,
            "rows": None,
            "columns": None,
        ...
        if series_summary["prefilter_pass"]:
            ...
            if converted_path and not convert_errors:
                ...
                if nifti_qc["nifti_qc_pass"]:
                    ...
                    ct_id = converted_name[:-7] if converted_name.endswith(".nii.gz") else Path(converted_name).stem
        ...
        report_row = {
            ...
            **series_summary,
            "image_type_union": ";".join(series_summary.get("image_type_union", []) or []),
            "prefilter_reasons": ";".join(series_summary.get("prefilter_reasons", []) or []),
            ...
            **totalseg_qc,
            ...
            "totalseg_detected_rois": ";".join(totalseg_qc.get("totalseg_detected_rois", []) or []),
            "totalseg_missing_rois": ";".join(totalseg_qc.get("totalseg_missing_rois", []) or []),
            "totalseg_roi_voxels": json.dumps(totalseg_qc.get("totalseg_roi_voxels", {}) or {}, ensure_ascii=False),
            "totalseg_roi_volumes_ml": json.dumps(totalseg_qc.get("totalseg_roi_volumes_ml", {}) or {}, ensure_ascii=False),
        }
        ...
        subject_id = safe_identifier(f"{case_id}__{ct_id.replace('.', '_')}")
        converted_file = Path(converted_path)
        file_size_mb = converted_file.stat().st_size / 1024**2 if converted_file.exists() else 0.0
        series_records.append(
            {
                "case_id": case_id,
                "ct_id": ct_id,
                ...
                **nifti_qc,
                **totalseg_qc,
                "image_type_union": list(series_summary.get("image_type_union", []) or []),
                "prefilter_reasons": list(series_summary.get("prefilter_reasons", []) or []),
...
def radqy_robust_z_flags(
    ...
    *,
    ...
    result["radqy_iqm_pass"] = pd.Series([count < int(fail_threshold) for count in bad_counts], dtype=object)
    return result
...
def link_dicom_series_for_radqy(source_dir: Path, target_dir: Path) -> bool:
    ...
    linked = 0
    for index, source_file in enumerate(sorted(path for path in source_dir.rglob("*") if path.is_file()), start=1):
...
def match_radqy_rows(series_records: list[dict[str, Any]], results_tsv: Path) -> list[dict[str, Any]]:
    ...
    for record in series_records:
        ...
        if not table.empty and participant_column in table.columns:
            matches = table[table[participant_column].astype(str).str.contains(subject_id, regex=False)]
            if not matches.empty:
                row = {str(key): value for key, value in matches.iloc[0].to_dict().items()}
        rows.append({**record, **row, "series_id": subject_id})
    return rows


def radqy_results_cover_series(series_records: list[dict[str, Any]], results_tsv: Path, metrics: Sequence[str]) -> bool:
    if not series_records or not results_tsv.is_file():
        return False
    matched = match_radqy_rows(series_records, results_tsv)
    for row in matched:
        if any(pd.isna(pd.to_numeric(row.get(metric), errors="coerce")) for metric in metrics):
            return False
    return True


def build_case_summary(case_id: str, series_records: list[dict[str, Any]], prefilter_report_path: str, results_tsv_path: str) -> dict[str, Any]:
    ranked_series = []
    for series in series_records:
        errors = []
        required = ["CV", "CJV", "EFC", "PSNR"]
        metrics_available = all(pd.notna(pd.to_numeric(series.get(metric), errors="coerce")) for metric in required)
        ...
        ranked_series.append({**series, "metrics_available": metrics_available, "series_errors": errors})

    ranked_series.sort(
        key=lambda item: (
            int(item.get("iqm_bad_count") or 999),
            float(pd.to_numeric(item.get("CV"), errors="coerce")) if pd.notna(pd.to_numeric(item.get("CV"), errors="coerce")) else float("inf"),
            float(pd.to_numeric(item.get("CJV"), errors="coerce")) if pd.notna(pd.to_numeric(item.get("CJV"), errors="coerce")) else float("inf"),
            float(pd.to_numeric(item.get("EFC"), errors="coerce")) if pd.notna(pd.to_numeric(item.get("EFC"), errors="coerce")) else float("inf"),
            -float(pd.to_numeric(item.get("PSNR"), errors="coerce")) if pd.notna(pd.to_numeric(item.get("PSNR"), errors="coerce")) else float("inf"),
            item.get("slice_thickness_median") if isinstance(item.get("slice_thickness_median"), (int, float)) else float("inf"),
            -int(item.get("n_slices") or 0),
        )
    )
    for index, series in enumerate(ranked_series, start=1):
        series["selection_rank"] = index
    selected = next((dict(series) for series in ranked_series if series.get("metrics_available") and series.get("radqy_iqm_pass")), {})
    ...
    if selected:
        ...
        selected_series = {
            ...
            "totalseg_total_roi_volume_ml": selected.get("totalseg_total_roi_volume_ml"),
    ...
    selected_entry = {
        ...
        "selected_source_file": str(selected.get("qa_source_path") or selected.get("ct_source_path") or ""),
    ...
    elif Path(prefilter_report_path).is_file():
        report = pd.read_csv(prefilter_report_path)
        if "totalseg_pass" in report.columns and (report["totalseg_pass"] == False).any():
            errors = ["no_valid_totalsegmentator_series"]
        elif "nifti_qc_pass" in report.columns and (report["nifti_qc_pass"] == False).any():
        ...
        "series_summaries": [
            {
                ...
                "totalseg_total_roi_volume_ml": series.get("totalseg_total_roi_volume_ml"),
    ...
    (case_output_dir / "selection_summary.json").write_text(json.dumps(to_jsonable(selection_summary), ensure_ascii=False, indent=2), encoding="utf-8")
    return selection_summary


def run_ct_qc_cohort(cases: list[Mapping[str, Any]], output_root: str = "", config_dir: str = "") -> dict[str, Any]:
    config = yaml.safe_load((Path(config_dir).expanduser() / "ct_qc.yaml").read_text(encoding="utf-8")) or {}
    ct_qc_dir = Path(output_root) / "ct_qc"
    ct_qc_dir.mkdir(parents=True, exist_ok=True)
    for path in [ct_qc_dir / "radqy_input", ct_qc_dir / "results.tsv"]:
        remove_path(path)

    with tempfile.TemporaryDirectory(prefix="path_ct_radqy_input_", dir="/tmp") as radqy_input:
        radqy_input_dir = Path(radqy_input)
        case_series: dict[str, list[dict[str, Any]]] = {}
        prefilter_paths: dict[str, str] = {}
        all_series = []
        show_progress = sys.stderr.isatty()
        for case in tqdm(cases, desc="CT QC cases", unit="case", disable=not show_progress):
            case_id = str(case.get("Case_ID", "") or case.get("case_id", "") or "unknown_case")
            case_output_dir = ct_qc_dir / case_id
            dcm2nii_dir = case_output_dir / "dcm2nii"
            case_output_dir.mkdir(parents=True, exist_ok=True)
            for path in [case_output_dir / "selection_summary.json", case_output_dir / f"{safe_identifier(case_id)}.nii.gz"]:
                remove_path(path)
            series_records, prefilter_report_path = prepare_ct_series(
                case_id,
                list(case.get("CT", []) or []),
                dcm2nii_dir,
            ...
            for record in series_records:
                ...
                subject_id = str(record.get("radqy_subject_id", "") or "")
                if source_dir.is_dir() and subject_id and link_dicom_series_for_radqy(source_dir, radqy_input_dir / subject_id):
                    all_series.append(record)

        radqy_config = dict(config.get("radqy", {}) or {})
        metrics_config = dict(radqy_config.get("metrics", {}) or {})
        radqy_metrics = [str(item) for item in list(metrics_config["high_bad"]) + list(metrics_config["low_bad"])]
        ...
        flagged_by_series_id = {str(row["series_id"]): dict(row) for _, row in flagged.iterrows()} if not flagged.empty else {}

        summaries = {}
        for case_id, records in tqdm(list(case_series.items()), desc="CT case selection", unit="case", disable=not show_progress):
            records = [flagged_by_series_id.get(str(record.get("radqy_subject_id", "") or ""), record) for record in records]
            summaries[case_id] = build_case_summary(case_id, records, prefilter_paths[case_id], str(results_tsv))
    return {
