...
def core_jaccard(reference, candidate):
    ...
    for left in reference.values():
        scores.append(max(len(set(left) & set(right)) / len(set(left) | set(right)) for right in candidate.values()))
...
def recovery_metrics(fused, patient_ids, cores):
    ...
    return {"core_recovery_mean_jaccard": mean_jaccard, "core_recovery_min_jaccard": min_jaccard, "fixed_core_mean_silhouette": float(silhouette_score(distance, labels, metric="precomputed"))}


def load_variants(data_root, patient_ids, canonical, snf_config):
    variants = {"canonical": (fuse(canonical, snf_config), {})}
    rna = pd.read_csv(data_root / "rna/case_features.csv").set_index("case_id").reindex(patient_ids)
    numeric = rna.apply(pd.to_numeric).fillna(0.0)
    ...
    for count in (1000, 3000, 5000):
        ...
        views = dict(canonical); views["rna"] = affinity(numeric[selected].to_numpy(), "correlation", snf_config)
        variants[f"rna_top_{count}"] = (fuse(views, snf_config), {"feature_count": len(selected), "view": "rna"})
    ...
    for name, columns in (("cnv_arm_only", [x for x in cnv if x.startswith("chr")]), ("cnv_arm_locus", [x for x in cnv if x.startswith("chr") or x.startswith("locus::")])):
        views = dict(canonical); views["cnv"] = affinity(cnv[columns].to_numpy(), "euclidean", snf_config)
...
def run(data_root, multi_k_root, config_dir, output_root, force=False):
    ...
    core_analysis.write_json(output_root / "feature_engineering_sensitivity_manifest.json", {"experiment": "feature_engineering_sensitivity", "patient_count": len(patient_ids), "stable_core_count": len(cores), "stable_core_patient_count": len(core_ids), "variants": list(variants), "wxs_prevalence_only_scope": "saved 22-gene discovery table; raw omitted WXS genes were unavailable for this offline audit", "core_recovery_definition": "fixed four stable-core membership recovered by deterministic K=4 clustering on the alternative fused affinity", "no_agent_rerun": True, "no_partition_replacement": True})
